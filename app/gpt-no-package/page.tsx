...
export default function GPTNoPackageReport() {
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/gpt-no-package');
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `gpt-no-package-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <h1 className="text-3xl font-bold text-[#f5c518] tracking-tight">Products with GPT Done - No Package</h1>
